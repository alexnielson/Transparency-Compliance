...
report_invalid_ucoa <- 
  entity_batches %>% 
  bind_rows() %>% 
  select(batch_id, entity_name, number_blank_or_na, non_17_digit, invalid_fund,
         invalid_funct, invalid_account_exp, invalid_account_rev) %>% 
  filter(!is.na(number_blank_or_na)  |
         !is.na(non_17_digit)        |
         !is.na(invalid_fund)        |
         !is.na(invalid_funct)       | 
...
report_invalid_dates %>%
  ...
  write_csv("posting_dates_highland.csv")
...
highland_invalid_summary <- 
  tibble(
    ...
    number_blank_or_na  = highland_ucoa %>%
                            filter(!is.na(number_blank_or_na)) %>% 
                            .[["number_blank_or_na"]] %>% 
                            sum(),
