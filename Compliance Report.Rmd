...
catch_during_next_cycle <- function(sf_id, initial_report_date, sf_status_history) {
  ...
  # Returns:
  # - 1 if the entity has uploaded a batch following the 60-day grace period and 0 
  #   otherwise.
  ...
  if (today() <= initial_report_date + days(60)) {
    ...
    if (any(transparency_status_history$last_uploaded_batch > (initial_report_date + days(60)), na.rm = TRUE)) {
      1
    } else {
      0
    }
...
>>>>>>> parent of a32e388... Removed code that referred to objects that don't exist. I meant to have the code remove the still_missing_noticed_* columns from the report data frame, but used rm() to do so, which is incorrect. As I use select() to pull the columns I want from report, it doesn't matter whether I remove those columns.
}
...
if (exists("initial_report_date")) {
  ...
  for (i in 1:nrow(report)) {
    ...
    # Alex programmed Salesforce to not send a 60-day notice to a government if 
    # that government has an unexpired temporary exemption, despite missing a 
    # required Transparency report. In order to avoid confusion, I've reflected
    # that feature in this code as well.
    if (report$unexpired_temporary_exemption[[i]] == 0 && (
        report$missing_exp[[i]] != "" ||
        report$missing_rev[[i]] != "" ||
        report$missing_w2[[i]]  != "")) {
      report$send_60_day_notice[[i]] <- 1
    } else {
