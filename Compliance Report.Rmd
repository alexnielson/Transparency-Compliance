...
```{r}
catch_during_next_cycle <- function(sf_id, 
                                    initial_report_date, 
                                    sf_status_history) {
  # Determine whether an entity that is still missing a noticed report has had
  # its hold removed due to a previously uploaded batch. If so, the batch did 
  # not contain the missing report and we will notice and enforce the missing
  # period during the next compliance cycle.
  #
  # Arguments:
  #   sf_id (character): The entity's Salesforce ID.
  #   initial_report_date (character): The date of the initial report of the 
  #     compliance cycle.
  #   sf_status_history (df): The dataframe containing entity Transparency 
  #     status information imported from Salesforce.
  # 
  # Returns:
  #   1 if the entity has at least one status of "Okay" following the 60-day
  #     grace period.
  #   0 if the entity does not have at least one status of "Okay" following the
  #     60-day grace period, or if the date of the compliance report is during
  ...
  if (today() <= initial_report_date + days(60)) {
    ...
    status_history <- 
      ...
      filter(salesforce_id == sf_id &
             valid_from_date > initial_report_date + days(60))
    
    if ("Okay" %in% status_history$status) {
      1
    } else {
      0
    }
...
SELECT
    ...
    Date_of_Last_Uploaded_Batch__c_hst AS last_uploaded_batch
...
sf_status_history <- 
  ...
  select(name, salesforce_id, status, valid_from_date, last_uploaded_batch)
...
for (i in 1:nrow(report)) {
  ...
  rm(still_missing_noticed_exp, still_missing_noticed_rev, 
   still_missing_noticed_w2)
}
