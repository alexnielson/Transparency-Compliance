...
*"Mail Merge v. 02" is based on "MailMerge Script," which was written by Alexandra Nelson.*
...
# reports than for me to wrangle the data and import it. Hopefully the OST will
# figure out a betterway to track the reports and send us a single .csv file
# next time):
...
duplicated_contacts <- 
  cfo[duplicated(cfo), ] %>% 
  select(entity_name, contact_name = cfo_name) %>% 
  bind_rows(cao[duplicated(cao), ] %>% 
              select(entity_name, contact_name = cao_name))

if (nrow(duplicated_contacts) > 0) {
  
  dlg_message("Salesforce has duplicate CFO and/or CAO information.
              Though duplicates will be removed in this program, review 
              duplicate_contacts and merge those contacts in Salesforce in order
              to enhance data quality.")
}
rm(duplicated_contacts)
  
cfo <- 
  cfo %>% 
  data.table() %>% 
  unique(by = c("entity_name", "cfo_name"))

cao <- 
  cao %>% 
  data.table() %>% 
  unique(by = c("entity_name", "cao_name"))
```


None of the entities that are in `account` and are missing an address have a CFO or CAO that has an address... So maybe there isn't a point to merging all that info.

```{r}
contact_info <- 
  account %>% 
  select(entity_name, street = account_street, city = account_city,
         state = account_state, zipcode = account_zipcode)

# missing_address <- 
#   account %>% 
#   filter(is.na(account_street)) %>%
#   select(entity_name) %>% 
#   left_join(cfo, by = "entity_name") %>% 
#   filter(is.na(cfo_street)) %>% 
#   select(-cfo_name) %>% 
...
#   left_join(cfo, by = "entity_name") %>% 
#   left_join(cao, by = "entity_name") %>% 
#   mutate(street = ifelse(is.na(account)))




  # mutate(address = ifelse(address.x == "NA NA, NA NA", address.y, address.x)) %>% 
  # select(-address.x, -address.y) %>% 
  # left_join(cao, by = "entity_name") %>% 
  # mutate(address = ifelse(is.na(address.x), address.y, address.x),
  #        cfo_name = ifelse(is.na(cfo_name), "Chief Financial Officer", cfo_name)) %>% 
  # select(-cao_name, -address.x, -address.y)
```
