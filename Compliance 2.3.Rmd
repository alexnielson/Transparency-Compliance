---
title: "Transparency Compliance 2.4"
...
initial_report_date <- mdy("8/1/2018")
```
...
```{r}
catch_during_next_cycle <- function(salesforce_id, initial_report_date, sf_status_history) {
  ...
  if (today() <= initial_report_date + days(60)) {
    ...
    transparency_status_history <- 
      sf_status_history %>% 
      filter(salesforce_id == salesforce_id) %>% 
...
if (exists("initial_report_date") &&
  ...
  report$grace_period_expired <- 1
  
} else {
    report$grace_period_expired <- 0
}
...
compliance_report <- 
  ...
  select(report_date, name, salesforce_id, missing_exp, missing_rev, missing_w2,
         ...
         still_missing_noticed_report, catch_next_cycle, analyst_note)
...
```{r, eval=FALSE}
vector_of_t_ids <- 
  c(623, 1496, 1635, 1634, 259, 221, 223, 833, 25, 283, 41, 205, 1175, 227, 107, 151, 153, 49)
...
compliance_report <- 
  ...
  select(report_date, name, salesforce_id, missing_exp, missing_rev, missing_w2,
         last_uploaded_batch, last_processed_batch, 
         unexpired_temporary_exemption, upload_in_queue, grace_period_expired,
         still_missing_noticed_report, catch_next_cycle, analyst_note)
