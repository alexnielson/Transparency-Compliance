...
initial_report_date <- as.Date("2019-02-04")
...
mark_catch_next_cycle <- function(sf_id, date, sf_list) {
  ...
  history_post_grace_period <- 
    ...
    filter(id == sf_id & valid_from_date > date + days(60))
...
mark_still_missing <- function(sf_id, sf_list, current_report) {
  ...
  current_status <- 
    sf_list$status_history %>% 
    filter(id == sf_id) %>% 
    filter(valid_from_date == max(valid_from_date))
  
  if (nrow(current_status) > 0                               &&
      (current_status$status[[1]] == "Delinquent - Enforced" ||
       current_status$status[[1]] == "On Hold"               ||
...
(identical(sf_independent_agency %>% nrow() %>% as.numeric(), 11)) ||
...
if (class(sf_status_history$name)            != "character" ||
    ...
    class(sf_status_history$valid_from_date) != "Date"      ||
...
subset_of_entities <- 
  c(133, 295, 445, 533, 623, 625, 1043, 1063, 1075, 1111, 1480, 1489, 1495, 
    1634, 1635)
```
...
if (names(initial_report_date) == "current_cycle" &&
  ...
  report <-
    ...
    mutate(
      ...
      catch_next_cycle    = map_dbl(sf_id,
                                    mark_catch_next_cycle,
