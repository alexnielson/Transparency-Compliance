...
initial_report_date <- as.Date("2019-05-01")
...
mark_catch_next_cycle <- function(sf_id, date, sf_list) {
  ...
  #   otherwise).
  # Comments:
  #   Following the 60-day grace period, we release an entity from begin on hold
...
mark_still_missing <- function(sf_id, sf_list, current_report) {
  ...
  #   compliance status is "Hold," "On Hold," or "Delinquent - Enforced."
  #   0 otherwise).
...
missing_reports <- function(t_id, begin_date, begin_fy, sf_id, exempt_list, trxn_type) {
  ...
  if (length(missing_reports) > 0) {
    ...
    if (trxn_type == 1 || trxn_type == 2) {
      ...
      if (trxn_type == 1) trxn_name <- "EXP"
      
      if (trxn_type == 2) trxn_name <- "REV"
...
# Can I combine this with the above query? Why do I do them separately? 
# I do them separately so I can more easily detect when there's a change in 
# Salesforce regarding independent agencies. The separate queries are by design.
...
# I created report_date to handle a different formating of t.Name.
...
# After talking with Alex, I realized that I should query data from 
# Transparency_Reporting__c rather than __c_hd.
# sf_status_history <- 
#   dbGetQuery(
#     odbc_sf,
#     paste("
...
#         Account__c_hst                      AS id,
#         Status__c_hst                       AS status
#       FROM Transparency_Reporting__c_hd
#       WHERE 
#         STRFTIME('%Y/%m/%d', ValidFromDate) >= 
#         STRFTIME(", report_date_as_character, ")")) %>%
#   left_join(sf_local_gov,
#             by = "id") %>% 
#   select(name, id, valid_from_date, status) %>% 
...
(identical(sf_independent_agency %>% nrow() %>% as.numeric(), 12)) ||
