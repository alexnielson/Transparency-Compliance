...
For non-archived Transparent Utah data, determine the percentage of UCoA values that are vague. 
...
```{r, eval=FALSE}
  #   reported_ucoa (tibble): The tibble output of query_reported_ucoa().
...
```{r}
query_reported_ucoa <- function(t_id) {
  # Query an entity's ucoa and type information for each reported transaction.
  #
  # Arguments:
  #   t_id (num): The entity's Transparency ID, as queried from Salesforce
  #     rather than Transparency.
  ...
  #
  # Comments:
  #   Some entities are missing the leading zero in the governmental activities,
  #   business-type activities, and discrete component unit fund numbers. This 
  #   function adds the leading zero back in.
  ...
  reported_ucoa <- 
    ...
    as_tibble() %>% 
    mutate(
      account_number = if_else(
                        nchar(account_number, allowNA = TRUE) == 18 &
                        substring(.$account_number, 1, 2) %in% 
                          (c(10:39) %>% as.character()), 
                        str_pad(account_number, width = 19, side = "left", "0"),
                        account_number))
}
```

## check_percentage_of_invalid_ucoa

```{r}
check_percentage_of_invalid_ucoa <- function(trxn_type, reported_ucoa, ucoa) {
  # Determine the percentage of an entity's transactions that use an invalid
  # UCoA code for the associated transaction type.
  #
  # Arguments:
  #   trxn_type (num): Transaction type (1 = expense, 2 = revenue).
  #   reported_ucoa (tibble): The tibble output of query_reported_ucoa().
  ...
  invalid_ucoa <- 
    ...
    filter(!substring(account_number, 1, 3) %in% ucoa$fund$NUMBER)
    
    # filter(!account_number %in% ucoa$account_exp$NUMBER)
...
extra_fund_numbers <- 
  c(11:19, 21:29, 31:39, 
    202:298, 302:398, 402:448, 452:498, 502:598, 602:698, 702:798) %>% 
  as.character() %>% 
  enframe(name = NULL) %>% 
  rename("NUMBER" = "value") %>% 
...
```{r}
reported_ucoa <- query_reported_ucoa(t_id)
...
# The error I get when I run the code below is:
...
check_percentage_of_vague_ucoa(299, "funct", ucoa)
```

```{r}
report_summary$percent_vague_function <- 
  report_summary$t_id %>% 
  map_dbl(check_percentage_of_vague_ucoa, "funct", ucoa)

report_summary$percent_vague_exp <- 
  report_summary$t_id %>% 
  map_dbl(check_percentage_of_vague_ucoa, "exp", ucoa)

report_summary$percent_vague_rev <- 
  report_summary$t_id %>% 
  map_dbl(check_percentage_of_vague_ucoa, "rev", ucoa)
```
