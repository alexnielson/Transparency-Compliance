...
* `account_number` meets the following requirements:
...
check_ucoa <- function(batch_id, govt_type, ucoa) {
  # Check whether a batch has valid Uniform Chart of Accounts codes (applicable
  # to only non-LEA and non-Higher Education entities).
  #
  # Arguments:
  #   batch_id (num): The batch's Transparency ID.
  #   govt_type (char): The type of government (e.g., City).
  ...
  if (govt_type == "School_District_or_Charter_School" ||
      govt_type == "Institute_of_Higher_Education") {
...
for (i in 1:nrow(report_invalid_batch)) {
  ...
  report_invalid_batch$number_invalid_account_exp[[i]] <- 
    invalid_batches %>% 
    filter(entity_name == report_invalid_batch$entity_name[[i]]) %>% 
...
for (i in 1:nrow(report_invalid_batch)) {
  ...
  report_invalid_batch$number_invalid_account_rev[[i]] <- 
    invalid_batches %>% 
    filter(entity_name == report_invalid_batch$entity_name[[i]]) %>% 
