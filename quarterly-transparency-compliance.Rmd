...
# 
# initial_report_date <- as.Date("2019-11-05")
# 
...
initial_report_date <- as.Date("2020-02-04")
...
```{r}
#convert_to_yyyy_mmm_mmm(2020.2, date("2019-07-01"))

fyyy.q <- 2020.2
start_month <- month(date("2019-07-01"))
  ...
  mmm_mmm <- 
    paste0(
      (date("2019-07-01") + months(first_month[str_detect(fyyy.q, qtrs)])) %>% 
        month(label = TRUE) %>% 
        toupper(),
      "-",
      (date("2019-07-01") + months(last_month[str_detect(fyyy.q, qtrs)])) %>% 
        month(label = TRUE) %>% 
        toupper())
...
missing_reports <- function(t_id, begin_date, begin_fy, sf_id, exempt_list, trxn_type) {
  ...
  print(paste("missing_reports - transaction_id: ",t_id))
...
# temp_data<- report
# 
# temp_batch_ids<- query_batch_ids( 223)
# 
# test_missing_rev <-
#   missing_reports(t_id = 223,
#                   begin_date = report$begin_report_er ,
#                   begin_fy = report$begin_fy,
#                   sf_id = report$id,
#                   trxn_type = 2,
#                   exempt_list = sf_data$exemptions)
# 
# test_reported_reports <-
#     query_batch_ids( report$t_id) %>%
#     map(query_posting_dates, report$begin_fy, trxn_type=2) #%>%
#     combine() %>%
#     convert_to_fyyy.q(report$begin_fy) %>%
#     unique()
# 
# 
# test_required_reports <-
#       required_fyyy.q(begin_date= report$begin_report_er,
#                       begin_fy= report$begin_fy,
#                       sf_id= report$id,
#                       trxn_type = 2,
#                       exempt_list = sf_data$exemptions)
# 
# 
#  missing_report <- setdiff(test_required_reports, test_reported_reports)
# 
#  temp <- convert_to_yyyy_mmm_mmm(missing_report, report$begin_fy)
...
query_last_processed_batch <- function(t_id) {
  ...
  if (is.na(t_id)) {
    ...
    if (nrow(last_processed_batch) == 0 || is.na(last_processed_batch[[1]])) {
      ...
      last_processed_batch[[1]] %>% as.Date()
    }
  }
}
```
  ...
  if (month(begin_fy) == 1) {
    ...
    dbGetQuery(
      ...
      mutate(
        ...
        fyyy.q = posting_date %>% convert_to_fyyy.q(begin_fy) %>% floor()) %>% 
      filter(fiscal_year == fyyy.q) %>% 
      ###ALEX  GOT RID OF FILTER TO FIX THE DAGGET CD ERROR
...
```{r}
# query_posting_dates(batch_id = 61261,
#                     begin_fy= report$begin_fy,
#                     trxn_type =2)
```
...
#       mutate(
#         fiscal_year = as.numeric(fiscal_year),
#         fyyy = posting_date %>% convert_to_fyyy.q(begin_fy) %>% floor()) %>% 
#       filter(fiscal_year == fyyy || fiscal_year == fyyy+1 || fiscal_year == fyyy-1)# %>% 
#       .[["posting_date"]] %>% 
#       as_vector()
# 
#       ### DEBUGG NOTES: 
#       ### I think that we can change the fiscal_year ==fyyy to include a standard deviation of 1 and the code should still run ok. It might be slightly slower, but it should not be much longer. this might solve the problem for the Dagget CD. We should test this out tomorrow. 
#       
# 
# temp_fyy.q <- convert_to_fyyy.q(date("2019-10-09"), begin_fy)
...
sf_status_history <- 
  ...
  left_join(sf_entity_info, by = "id") %>% 
...
## check that correct field data classes
...
## check that all entities have a name and Salesforce id
if (sf_exemptions %>% 
    filter(is.na(name) | is.na(id)) %>% 
    nrow() > 0) {
...
## check that the begin_fy is up to date. 
if (sf_exemptions %>% 
    filter(begin_fy <= (today() - months(12))) %>% 
    nrow() > 0) {
...
#check that the exemption start date is not after the exemption end date
if (sf_exemptions %>% 
...
# check that if a entitiy has reccuring exmption, that it has both start and end
# dates available. 
if (sf_exemptions %>% 
    filter(
...
#check that a temporary exmption has both start and end dates.
if (sf_exemptions %>% 
    filter(
...
#check w-2 exemptions are not reccuring. 
if (sf_exemptions %>% 
    filter(
      recurring_exemption == TRUE &
...
#check that w-2 exemption matches first month of entity fiscal year
if (sf_exemptions %>% 
    filter(
...
needs_extra_attention <- 
  ...
  filter(
    ...
    (last_uploaded_batch  < Sys.Date() %m-% months(1) |
     last_processed_batch < Sys.Date() %m-% months(1)))
...
if (!exists("subset_of_entities")) {
  ...
  compliance_report %>%
    # Reformat date fields to match format in Salesforce (mm/dd/yyyy):
    mutate(
      last_processed_batch = last_processed_batch %>% format("%m/%d/%Y"),
      last_uploaded_batch  = last_uploaded_batch %>% format("%m/%d/%Y")) %>% 
    write_csv(
