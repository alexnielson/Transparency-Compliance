---
title: "Transparency Compliance 2.2 - Single Entity"
...
For the active and participating entity that OSA will monitor for Transparency compliance, report the fiscal periods for which the entity has not complied with the following reporting requirements and does not have a permanent reporting exemption:
...
The only user inputs required for this program are the the entity's Transparency ID, the DSN names to Salesforce and AWS, and, if this not the first Compliance Report of the enforcement cycle, the date of the initial report.

```{r}
entity_transparency_id <- 1391
dsn_sf                 <- "Salesforce"
dsn_aws                <- "transpAWS"
```
...
```{r}
initial_report_date <- paste0("'", "8/1/2018", "'")
```
...
if (exists("initial_report_date")) {
  initial_report <- 
    dbGetQuery(
      odbc_sf,
      paste("
            SELECT t.Name AS initial_report_date,
            ...
            FROM (
              SELECT Name, Account__c, Expenditure_Problem__c,
                Revenue_Problem__c, Wage_Problem__c, Upload_In_Queue__c, 
                Analyst_Notes__c
              FROM Transparency_Reporting__c
              WHERE Name = ", initial_report_date, ") t
            LEFT JOIN Account a
            ON t.Account__c = a.Id")) %>% 
    filter(upload_in_queue == FALSE)
}
```
...
if (nrow(outdated_fiscal_year_begin) > 0) {
  stop("fiscal_year_begin must reflect the current fiscal year, rather than a prior fiscal year. 
       Update data in Salesforce and reimport (or use the update_old_fiscal_years function).", call. = FALSE)
}
...
Correct outdated fiscal_year_begin values using update_old_fiscal_years(), if not doing so in Salesforce:
...
report <- 
  sf_info %>% 
  filter(transparency_id == entity_transparency_id) %>% 
  as_tibble(sf_info)
...
compliance_report <- 
  report %>% 
  select(name, salesforce_id, missing_exp, missing_rev, missing_w2,
         last_uploaded_batch, last_processed_batch, analyst_note,
         unexpired_temporary_exemption, upload_in_queue, 
         still_missing_noticed_report)
