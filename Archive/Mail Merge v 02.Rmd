...
reports__c <- 
  dbGetQuery(
    ...
    -- We send a 10-day notice to entities with missing budgets. As the Office
    -- Specialist (Brooke) is responsible for creating that report, I won't
    -- query those entities in this program.
    AND r.Name NOT IN ('Approved Budget', 'Amended Budget')
...
# Alex had the summarize_all() and mutate_all() functions piped as the last two
# lines in the code above. I'm not sure why, as it looks like she removed the NA
# values just to add them back in again with the next line of code. I'll keep it
# for now, in case I discover why she did this.
  #  %>% 
...
In the code below, I filter the data using `max(created_date)`. I expect this to work without problems in this program, as the trigger for running this Mail Merge program is running an initial or post-60-day grace period report. Meaning, every active and participating entity should have the same value for `created_date` prior to running the Mail Merge program.

The filter condition would cause a problem in instances when any one entity has a more-recently created Compliance Report than the others (such as when happens when an entity is on hold, submits an upload to Transparent Utah, and then emails or calls us to request that we remove the hold status.) I don't expect we would run the Mail Merge program following these type of events, so I won't spend time trying to correct a problem that I don't expect to occur.
...
missing_reports <- 
  missing_reports_osa %>% 
  full_join(missing_reports_transp, by = "entity_name")

report_send_notices <- 
  contact_info %>% 
  right_join(missing_reports, by = "entity_name")

duplicate <- duplicated(report_send_notices$entity_name)

report_send_notices <- 
  report_send_notices[!duplicate, ]
...
if (notice_type == "60_day_notice") {
  ...
  report_send_notices %>% 
    write_csv(
      path = paste0(file_path, Sys.Date(), " Mail Merge 60-day Notice.csv"),
      na = ""
    )
  ...
  report_send_notices %>% 
    ...
    write_csv(
      path = paste0(file_path, Sys.Date(), " Mail Merge Hold Notice.csv"),
      na = "")
  
  report_send_notices %>% 
    filter(non_taxing_entity == TRUE) %>% 
    write_csv(
      path = paste0(file_path, Sys.Date(), " Mail Merge Suspend Notice.csv"),
      na = "")
}
```
