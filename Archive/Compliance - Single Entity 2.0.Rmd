---
title: "Transparency Compliance 2.0 - Single Entity"
...
The only user inputs required for this program are the the entity's Transparency ID and the DSN names to Salesforce and AWS.

```{r}
entity_transparency_id <- 1529
dsn_sf                 <- "Salesforce"
dsn_aws                <- "transpAWS"
```
...
report$analyst_note <- 
  paste(report$note_1, report$note_2)
...
compliance_report <- 
  ...
  select(name, salesforce_id, missing_exp, missing_rev, missing_w2,
         last_uploaded_batch, last_processed_batch, unexpired_temporary_exemption,
         upload_in_queue, analyst_note)
